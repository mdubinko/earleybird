use std::fs;
use argh::FromArgs;
use earleybird::testsuite_utils::{TestCatalog, self};
...
impl RunSuite {
    pub fn run(self) {
        ...
        //println!("{:?}", r);
        //println!("===");
        if let Result::Ok(html) = r {
            println!("{:?}", html);
        }

        let ts = read_test_catalog(self.dir);
        println!("==");
        println!("{:?}", ts);
...
fn read_test_catalog(path: String) -> Result<TestCatalog, DeError> {
    //let file = fs::read_to_string(path).expect("The file could not be read");

    let file = testsuite_utils::TEST_CATALOG_EG;
    println!("{file}");
    let test_catalog = from_str(&file)?;
    println!("...........");
    Ok(test_catalog)
}
