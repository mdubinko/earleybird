use crate::{
    grammar::{Grammar, Mark, RuleContext, TMark, TerminalDefn},
    testsuite_utils::{TestCase, TestGrammar, TestResult},
};
use indoc::indoc;
    ...
    fn add<T: ParserTestSet>(&mut self, testset: &T) {
        ...
        for (input, expected) in ins_outs {
            self.tests.push(TestCase {
                name: name.to_owned(),
                grammars: vec![
                    TestGrammar::Unparsed(ixml.to_string()),
                    TestGrammar::Parsed(grammar.clone()),
                ],
...
impl ParserTestSet for SmokeStarSep {
    ...
    fn get_grammar(&self) -> Grammar {
        ...
        g.define(
            "doc",
            ctx.seq().repeat0_sep(ctx.seq().ch('a'), ctx.seq().ch(' ')),
        );
        g
    }
    fn get_inputs_expected(&self) -> Vec<(&'static str, &'static str)> {
...
impl ParserTestSet for SmokePlusSep {
    ...
    fn get_grammar(&self) -> Grammar {
        ...
        g.define(
            "doc",
            ctx.seq().repeat1_sep(ctx.seq().ch('a'), ctx.seq().ch(' ')),
        );
        g
    }
    fn get_inputs_expected(&self) -> Vec<(&'static str, &'static str)> {
        vec![("a a", "<doc>a a</doc>"), ("a a a", "<doc>a a a</doc>")]
    }
}
...
impl ParserTestSet for SmokeAttr {
    ...
    fn get_grammar(&self) -> Grammar {
        ...
        g.mark_define(
            Mark::Attr,
            "name",
            ctx.seq().repeat1(ctx.seq().ch_range('a', 'z')),
        );
        let ctx = RuleContext::new("value");
        g.define("value", ctx.seq().repeat1(ctx.seq().ch_range('a', 'z')));
        g
